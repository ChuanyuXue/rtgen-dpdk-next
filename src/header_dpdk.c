...
void setup_ethernet_header(struct rte_mbuf *pkt,
                           struct rte_ether_addr *src_addr,
                           struct rte_ether_addr *dst_addr,
                           int vlan_enabled, int udp_ip_enabled,
                           uint16_t vlan_id, uint16_t vlan_priority) {
    struct rte_ether_hdr *eth_hdr = (struct rte_ether_hdr *)rte_pktmbuf_append(
    ...
    if (vlan_enabled) {
        ...
        struct rte_vlan_hdr *vlan_hdr = (struct rte_vlan_hdr *)rte_pktmbuf_append(
            pkt, sizeof(struct rte_vlan_hdr));
...
struct rte_ipv4_hdr *setup_ip_header(struct rte_mbuf *pkt, uint32_t src_ip,
                                     uint32_t dst_ip, size_t data_size) {
    struct rte_ipv4_hdr *ip_hdr = (struct rte_ipv4_hdr *)rte_pktmbuf_append(
        pkt, sizeof(struct rte_ipv4_hdr));
    ip_hdr->version_ihl = 0x45;
    ip_hdr->type_of_service = 0;
    ip_hdr->total_length = rte_cpu_to_be_16(
        sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr) + data_size);
    ip_hdr->packet_id = 0;
...
struct rte_udp_hdr *setup_udp_header(struct rte_mbuf *pkt, uint16_t src_port,
    ...
    struct rte_udp_hdr *udp_hdr = (struct rte_udp_hdr *)rte_pktmbuf_append(
