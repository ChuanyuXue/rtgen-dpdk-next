...
void apply_command_arguments(struct flow_state *state)
{
  struct interface_config *config = create_interface(
      0, interface, "", address, port, port);
  /* No offset from the base pit_offset for single flow */
  struct flow *flow = create_flow(
      pit_payload_size, pit_priority, pit_delta, pit_period, 0,
      0, config);
  add_flow(state, flow);
}
...
void apply_config_file(struct flow_state *state, const char *config_path)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fp) != NULL)
  {
    ...
    struct interface_config *config = create_interface(
        vlan_id, interface, "", address, port, port);

    struct flow *flow = create_flow(
        size, priority, delta, period, begin_high, begin_low, config);
    add_flow(state, flow);
  }
};
...
int main(int argc, char *argv[])
{
  ...
  /*   Initialize the DPDK port
       -> this model assumes each flow has a dedicate interface
  */
  ...
  while (curr_time.tv_sec - start_time < pit_runtime)
  {
    ...
    for (int i = 0; i < schedule->num_frames_per_cycle; i++)
    {
      ...
      uint64_t sche_time = flow->sche_time->tv_sec * ONESEC + flow->sche_time->tv_nsec;
      ...
      if (pit_loopback == 1)
      {
        ...
        printf("SW-SCHE    TIMESTAMP %ld.%09ld\n", flow->sche_time->tv_sec, flow->sche_time->tv_nsec);
      }
      sche_single(fd, address, port, sche_time - MAC_DELAY,
                  message, flow->size);
